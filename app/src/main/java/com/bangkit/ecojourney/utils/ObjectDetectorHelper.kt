...
class ObjectDetectorHelper(
    ...
    init {
        TfLiteGpu.isGpuDelegateAvailable(context).onSuccessTask { gpuAvailable ->
            ...
            context?.getString(R.string.tflitevision_is_not_initialized_yet)
                ?.let { it1 -> detectorListener?.onError(it1) }
        }
    }
    ...
    private fun setupObjectDetector() {
        ...
        try {
            ...
            context?.let { detectorListener?.onError(it.getString(R.string.image_classifier_failed)) }
    ...
    fun detectObject(image: ImageProxy) {
        ...
        if (!TfLiteVision.isInitialized()) {
            ...
            detectorListener?.onError(errorMessage?:"Error")
            return
        }
